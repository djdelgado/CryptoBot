...
var getCoinInfo = function (coin) {
	...
	Bot.cryptoGet(pathUrl, ({ data }) => {
		...
		for(var i in data){
			if(uppercase(data[i].id) === coinRequest || uppercase(data[i].name) === coinRequest || uppercase(data[i].symbol) === coinRequest){
				console.log(data[i]);
				sendInfo.push("Ticker: " + data[i].symbol + " " + data[i].name);
				sendInfo.push("Price: " + "$" + unround(data[i].price_usd));
				sendInfo.push("24hr: " + data[i].percent_change_24h + "pct");
				sendInfo.push("7d: " + data[i].percent_change_7d + "pct");
				sendInfo.push("Mkt Cap: " + "$" + data[i].market_cap_usd.toString());
