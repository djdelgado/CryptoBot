...
var getCoinInfo = function (coin) {
	...
	Bot.cryptoGet(pathUrl, ({ data }) => {
		...
		const unround = (str) => parseFloat(str) < 0 ? parseFloat(str) : parseFloat(str).toLocaleString('en-US');
		const coinRequest = uppercase(coin);
		
		for(var i in data){
			if(uppercase(data[i].name) === coinRequest || uppercase(data[i].symbol) === coinRequest){
				console.log(data[i]);
				sendInfo.push("Ticker: " + data[i].symbol + " " + data[i].name);
				sendInfo.push("Price: " + "$" + unround(data[i].quote.USD.price));
				sendInfo.push("24hr: " + data[i].quote.USD.percent_change_24h + "pct");
				sendInfo.push("7d: " + data[i].quote.USD.percent_change_7d + "pct");
				sendInfo.push("Mkt Cap: " + "$" + unround(data[i].quote.USD.market_cap));
